...
namespace lab5_6
{
    ...
    public class Transport : BaseClone, ICloneable, ICloneable1
    {
        ...
        public int Price 
        {
            ...
            set
            {
                if (value <= 0)                                                         //если значение меньше 0 тогда
                    throw new Exceptions.PriceException("Цена не может быть меньше 0"); //исключение priceexception 
                else
                    price = value;
            }
        }
        ...
        public int MaxSpeed
        {
            ...
            {
                if (value > 250)
                    throw new Exceptions.SpeedException("Скорость не может принимать такое значение", value); //исключение
                else
                    max_speed = value;
            }
        }
    ...
    public class TransportAgency
    {
        ...
        {
            throw new Exceptions.NotImplement("Неправильное значение"); //исключение
        }
    }
    public static class TransportController
    {
        ...
        {
            Debug.Assert(list != null, "список не может быть пустым");          //макрос assert проверяет список
    ...
    class Exceptions                                                            //иерархия классов исключений
    {
        public class PriceException : Exception                 //наслед. пользоват.типов от стандартных
        {
