...
namespace lab8
{
    ...
    interface IOperations<T>                    //обобщенный интерфейс с операциями 
    {
    ...
    public class MyList<T> : IEnumerable<T>, IOperations<T> where T : new()  //обобщенный класс в котором наслед интерф
    {
        public int Capasity;
        public string ItemOfList;
        public string itemoflist { get; set; }
        List<T> list = new List<T>();
        public void AddItem(T element)              // метод IOperations
        {
            list.Add(element);
        }
        public void DeleteItem(int position)        //метод IOperations
        {
        ...
        Owner owner = new Owner("User1", "BSTU");
        ...
        public bool Equals(MyList<T> list) // сравнение
        {
    ...
    class Program
    {
        ...
        {
            try                                                         //обработка исключений с finally
            {
                MyList<int> list1 = new MyList<int>();                  //исп обобщ для станд типов
                MyList<double> list2 = new MyList<double>();
                MyList<Transport> list3 = new MyList<Transport>();      //исп обобщ для пользов типов
                list1.AddItem(1);
                list1.AddItem(2);
                list1.AddItem(3);
                list1.DeleteItem(3);
                ...
                list3.DeleteItem(1);
                list3.Show();
                FileStream file = new FileStream("e:\\newtext.txt", FileMode.Create);           //создание файла
                StreamWriter writer = new StreamWriter(file);                                   //сохранение в файл
            ...
            finally                                                                     //обработка исключений с finally
            {
